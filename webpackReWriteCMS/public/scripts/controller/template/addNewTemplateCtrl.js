/**
 * Created by wuyunge on 2015/4/5.
 * 与发邮件的controller不同之处在$scope.which_part：937
 */
"use strict";
innerApp.controller('add_new_template_ctrl',
    ["$scope", "$rootScope",
        ...
        function ($scope, $rootScope, templateGeneratorInfo,
            ...
            var viewProperty = $scope.viewProperty = {
                ...
                mail: {
                    recipient_info: '',
                    sender_info: ($rootScope.global.me.display_name || $rootScope.global.me.name || '') + ( ($rootScope.global.me.reply_email || $rootScope.global.me.email) ? '<' + ($rootScope.global.me.reply_email || $rootScope.global.me.email) + '>' : '' ),
                    title_info: '',
            ...
            /**
             * 视图的转换器
             * @type {{status: string, changeTo: Function}}
             */
            $scope.viewProperty.switcher = {
                status: 'empty',
                show_siderbar: false,
                toggleSiderbar: function () {
                  this.show_siderbar = !this.show_siderbar;
                },
                changeTo: function (status) {
                    function syncStatus(status) {
                        if (status === 'empty') {
                            $scope.viewProperty.switcher.show_siderbar = false;
                            $scope.viewProperty.custom_template__content = '';
                            $scope.viewProperty.custom_template__title = '';
                        }
                        if (status === 'automatic') {
                            $scope.viewProperty.switcher.show_siderbar = true;

                            $rootScope.global.left_panel_is_fold = true;

                            $scope.viewProperty.custom_template__content = generator.generate();
                            $scope.viewProperty.custom_template__title = generator.generateTitle();
                        }
                    }
            ...
            if ($rootScope.global.is_login && $scope.viewProperty.switcher.status !== 'empty') {
...
//                    }
            ...
            $rootScope.$on(kzi.constant.event_names.mail_box_show, function (e, i) {
                //从窗口偷翻进来的
                if (i.from == "groupBatch" && i.projectInfos) {
                    ...
                    $scope.viewProperty.fromWhere = 'groupBatch';
