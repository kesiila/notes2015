'use strict';
innerApp.controller('orders_ctrl', ["$http","$rootScope", "$scope", "$routeParams", "$popbox", "$timeout", "$location",
function ($http,$rootScope, $scope, $routeParams, $popbox, $timeout, $location) {
        //初始化数据
        var init=function() {
            $rootScope.global.title = "订单管理";
            $rootScope.global.loading_done=false;

            //获取类别
            $http.get('/api/codes/order/all').success(function (response) {
        ...
        $scope.save=function()
        {
            ...
            {
                wt.data.order.create($scope.new_element, function ()
                    {
                    	$scope.is_saving=true;
                        kzi.msg.success("订单保存成功！",function(){});
                        $scope.isDESC=1;
                        $scope.queryCondition.isDESC=$scope.isDESC;
                        $scope.cancelForm();
                        get_page($scope.queryCondition);
                    },
                    function(){kzi.msg.error("订单保存失败！",function(){});},
                    function (){$scope.is_saving=false;});
            }
        };

        //修改订单事件
        $scope.$on(kzi.constant.event_names.on_order_update, function (e, i) {
            if (!_.isEmpty(i)) {
                var n = _.findWhere($scope.items, { order_id: i.order_id });
                n && (n.name = i.name, n.typeName= i.s_typeName, n.summary = kzi.helper.substr(i.content, 60));
            }
        });

        //删除订单事件
        $scope.$on(kzi.constant.event_names.on_order_trash,function(e, i){
            if (!_.isEmpty(i))
            {
        ...
        //删除评论事件
        $scope.$on(kzi.constant.event_names.on_order_comment_del,function(e, i){
            if (!_.isEmpty(i))
            {
                var n = _.findWhere($scope.items, {order_id:i});
                n && n.comment_count--;
            };
        });

        //添加评论事件
        $scope.$on(kzi.constant.event_names.on_order_comment,function(e, i){
            if (!_.isEmpty(i))
            {
                var n = _.findWhere($scope.items, i);
                n && n.comment_count++;
